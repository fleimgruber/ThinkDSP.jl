...
# ╔═╡ 499d7c20-7a9b-11ec-22eb-a9dddd35d910
begin
	import Pkg
	dev = Pkg.develop
	dev(path="C:/Users/LeimgruberF/dev/ThinkDSP.jl")
	using Revise
	using ThinkDSP
	using Plots
end

# ╔═╡ 2904c205-3ed0-4504-9160-e7de9a14a7f7
begin
	cos_sig = Signal(freq=440,amp=1,offset=0,f=cos,name="cossignal1")
	sin_sig = Signal(freq=880,amp=0.5,offset=0,f=sin,name="sinsignal1")
end

# ╔═╡ 7f2b46c5-c5f3-484b-a4a0-d175027a4a23
mix = sin_sig + cos_sig

# ╔═╡ bb56bc60-7cda-4ced-97c0-ba211b20655f
latexify(sin_sig)

# ╔═╡ 73f644c6-6c27-4fc4-9222-7ef9d8407ffa
mix + mix

# ╔═╡ 1f30d31f-8c10-4edd-9a0a-ed7e40f2c33a
period(sin_sig)

# ╔═╡ 6ee0f393-42be-4959-92cf-ed38720ad9cb
period(mix)

# ╔═╡ f69e5a33-0c73-4850-a0e3-3f347300c53e
wave = Wave(mix; framerate=11025)

# ╔═╡ e3b7403d-9398-42e7-ae80-98c36bf204ee
plot(wave)

# ╔═╡ 48064689-e254-4237-9da5-40ef50a82ccb
p = period(mix)

# ╔═╡ 6c91f41e-1dbe-45ee-a70e-4d68a4384de1
plot(Wave(mix; t=3*p, framerate=11025))

# ╔═╡ 5a56363b-2f89-4405-abf7-af991cf15d18
begin
	signal1 = Signal(freq=100,amp=1,offset=0,f=sin,name="sinsignal1")
	wave_noise = Wave(signal1 + BrownianNoise(1) + WhiteNoise(0.1); t=3) 
	plot(wave_noise)
end

# ╔═╡ 1f673cf2-31df-4453-bd58-8dc985fea84b
begin
	using WAV
	wavplay(wave_noise)
end

# ╔═╡ 3dd03c4f-bd1b-4633-b793-ca8aaac30fd8
wave_noise.framerate

# ╔═╡ cd7e7f4a-6396-46f7-92fc-2198b7cd2713
wave_noise.amps

# ╔═╡ 6221c82e-563c-4ab8-bca5-8edee6567b9b
wavwrite(wave_noise.amps, "example.wav", Fs=wave_noise.framerate)

# ╔═╡ f97db77f-5951-4848-a879-fcbc71b50991
plot(Wave(Spectrum(wave_noise)))
...
ThinkDSP = "6f99ff31-3093-4113-84a5-03ad7dc5a3e2"
...
git-tree-sha1 = "7a710fce4787216003ccd45b804772be20fc7258"
...
[[ThinkDSP]]
deps = ["FFTW", "JuliaFormatter", "LaTeXStrings", "Latexify", "LinearAlgebra", "Plots", "Pluto", "PlutoUI", "RecipesBase", "Reexport", "Revise", "Statistics", "Symbolics", "WAV"]
path = "C:/Users/LeimgruberF/dev/ThinkDSP.jl"
uuid = "6f99ff31-3093-4113-84a5-03ad7dc5a3e2"
version = "0.1.0"
